...
window.changeAvatar = function(profileId, currentAvatar) {
  ...
  avatarSelector.innerHTML = `
    ...
    <div style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 1rem; margin-bottom: 1rem;">
      ${AVATARS.map(avatar => `
        <div 
          style="font-size: 3rem; text-align: center; cursor: pointer; padding: 0.5rem; border: 2px solid ${avatar === currentAvatar ? '#fff' : 'transparent'}; border-radius: 4px; transition: all 0.2s;"
