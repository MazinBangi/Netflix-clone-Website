...
// Fixed Profile-based Watchlist functionality
const API_URL = "/api";
...
// Fetch watchlist from PROFILE (not user)
async function loadWatchlist(query = "", genre = "") {
  ...
  try {
    ...
    // Use PROFILE-based watchlist endpoint
    const res = await fetch(`${API_URL}/profiles/${currentProfileId}/watchlist`, {
    ...
    console.log("Profile watchlist data:", data);
...
function renderWatchlist(movies) {
    ...
    card.innerHTML = `
      ...
      <div class="meta">
        ...
        <div class="card-actions">
          ...
          <button class="btn-remove" data-id="${movie._id}" data-title="${movie.title}">
  ...
  watchlistGrid.querySelectorAll(".btn-remove").forEach(btn => {
    btn.addEventListener("click", async (e) => {
      ...
      if (!confirm(`Remove "${movieTitle}" from ${currentProfileName}'s list?`)) {
        return;
      }
      ...
      try {
        ...
        // Use PROFILE-based remove endpoint
        ...
        if (res.ok) {
          showNotification(`"${movieTitle}" removed from ${currentProfileName}'s list!`);
...
btnClearAll?.addEventListener("click", async () => {
  if (!watchlistData.length) {
    showNotification(`${currentProfileName}'s list is already empty`, "error");
    return;
  }
  
  if (!confirm(`Are you sure you want to clear ${currentProfileName}'s entire watchlist? This action cannot be undone.`)) {
    return;
  }
  
  try {
    btnClearAll.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Clearing...';
    btnClearAll.disabled = true;
    
    // Use PROFILE-based clear endpoint
    const res = await fetch(`${API_URL}/profiles/${currentProfileId}/watchlist/clear`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` }
    });
    
    if (res.ok) {
      showNotification(`${currentProfileName}'s watchlist cleared successfully!`);
      loadWatchlist();
    } else {
      const errorData = await res.json();
