...
export const createProfile = async (req, res) => {
  try {
    ...
    if (name.length > 20) {
      return res.status(400).json({ message: "Profile name must be 20 characters or less" });
    }
    ...
    res.status(500).json({ message: error.message || "Error creating profile" });
  }
};
...
export const addToProfileWatchlist = async (req, res) => {
  try {
    ...
    const updatedProfile = await Profile.findById(profileId).populate("watchlist");
